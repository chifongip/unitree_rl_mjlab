...
**`feet_distance`**: Hinge penalty on XY distance between left and right foot sites. Cost = `clamp(min_distance - xy_dist, min=0) / min_distance`. Always active (standing and walking). Default `min_distance=0.15m`; default stance width is ~0.237m (all heights, hip_roll=0). Penalizes inward hip_roll before feet get dangerously close. Logs `Metrics/feet_distance_mean`.
