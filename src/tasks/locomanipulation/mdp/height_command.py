...
class BaseHeightCommand(CommandTerm):
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        if self.cfg.fixed_height is not None:
            ...
            return
        lo = self.cfg.nominal_height - self.cfg.max_deviation_down * self.cfg.height_scale
        hi = self.cfg.nominal_height + self.cfg.max_deviation_up * self.cfg.height_scale
...
class BaseHeightCommandCfg(CommandTermCfg):
    ...
    max_deviation_down: float = 0.285
    """Maximum downward deviation from nominal (meters). nominal - min(ranges)."""
    max_deviation_up: float = 0.0
    """Maximum upward deviation from nominal (meters). max(ranges) - nominal."""
