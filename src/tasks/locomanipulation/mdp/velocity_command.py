...
class UniformVelocityCommand(CommandTerm):
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    if self.cfg.fixed_command is not None:
      self.vel_command_b[env_ids, 0] = self.cfg.fixed_command[0]
      self.vel_command_b[env_ids, 1] = self.cfg.fixed_command[1]
      self.vel_command_b[env_ids, 2] = self.cfg.fixed_command[2]
...
class UniformVelocityCommandCfg(CommandTermCfg):
  ...
  """If set, always use (lin_vel_x, lin_vel_y, ang_vel_z) instead of random sampling."""
