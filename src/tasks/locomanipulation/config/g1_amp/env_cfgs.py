"""Unitree G1 AMP locomanipulation environment configurations."""
...
from src.tasks.locomanipulation.locomanipulation_amp_env_cfg import (
  make_locomanipulation_amp_env_cfg,
)
from src.tasks.locomanipulation.config.g1.env_cfgs import (
  MOTION_DOF_INDICES,
  LOWER_BODY_JOINT_NAMES,
  LOWER_BODY_JOINT_PATTERNS,
  LOWER_BODY_JOINT_CFG,
)


def unitree_g1_locomanipulation_amp_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create Unitree G1 rough terrain AMP locomanipulation configuration."""
  cfg = make_locomanipulation_amp_env_cfg()
  ...
  motion_file = str(SRC_PATH / "assets" / "data" / "g1" / "bones_seed.pkl")
  ...
  ## Observations — AMP (lower-body only, anchored to pelvis) ##
  ...
  for term_key in ("body_pos_b", "body_ori_b", "body_lin_vel_b", "body_ang_vel_b"):
    cfg.observations["amp"].terms[term_key].params["anchor_cfg"].body_names = (amp_anchor,)
    cfg.observations["amp"].terms[term_key].params["body_cfg"].body_names = amp_body_names

  ## Events — motion data path ##

  _motion_dir = os.path.normpath(
    os.path.join(
      os.path.dirname(__file__), "..", "..", "..", "..",
      "assets", "motions", "g1", "amp", "WalkandRun",
    )
  )
  ...
  if play:
    ...
    #     "left_wrist_yaw_link": {"x": 5.0, "y": -5.0, "z": -20.0},
    #     "right_wrist_yaw_link": {"x": 5.0, "y": 5.0, "z": -20.0},
    # }
...
def unitree_g1_locomanipulation_amp_flat_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create Unitree G1 flat terrain AMP locomanipulation configuration."""
  cfg = unitree_g1_locomanipulation_amp_rough_env_cfg(play=play)
